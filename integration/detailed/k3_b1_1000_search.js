...
const A=require('./k3_architecture_search'),{simulate,MiB}=require('./k3_operator_sram_sim.js');
...
<p><a href="../../integration/detailed/k3_b1_1000_search.js">单目标搜索程序</a> · <a href="../../out/search/k3_b1_1000_results.json">全部参数、结果和93层账 JSON</a></p><div class="formula">B1_SCALE=${d.scale} node integration/detailed/k3_b1_1000_search.js
node tests/regression/test_k3_b1_1000_search.js
# 保持同样硬约束，只增加搜索次数
B1_SCALE=2 node integration/detailed/k3_b1_1000_search.js</div>
...
function run(){
 ...
 const old=JSON.parse(fs.readFileSync('out/search/k3_architecture_search_results.json','utf8'));
 ...
 d.inputHash=crypto.createHash('sha256');for(const f of ['integration/detailed/k3_b1_1000_search.js','integration/detailed/k3_architecture_search.js','integration/detailed/k3_operator_sram_sim.js','teams/model/src/design_engine.js','out/search/k3_architecture_search_results.json'])d.inputHash.update(fs.readFileSync(f));d.inputHash=d.inputHash.digest('hex');d.seconds=(Date.now()-t)/1000;
 fs.writeFileSync('out/search/k3_b1_1000_results.json',JSON.stringify(d,null,2));fs.writeFileSync('out/search/k3_b1_1000_report.html',report(d));
...
module.exports={TARGET,EXT,evaluate,search,diagnostics,report,run};if(require.main===module)run();
