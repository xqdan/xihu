...
const {search,evaluate,pareto,SPACE}=require('../detailed/k3_architecture_search.js');
...
const result=renderOnly?JSON.parse(fs.readFileSync('out/search/k3_architecture_search_results.json','utf8')):search(opt);
...
const t=fs.readFileSync('integration/templates/search/k3_architecture_search_report_template.html','utf8');
fs.writeFileSync('out/search/k3_architecture_search_report.html',t.replace('/*__REPORT_DATA__*/',JSON.stringify(result).replace(/</g,'\\u003c')));
