...
const baseline = read('docs/design/spec/k3_mc_baseline.json');
const sourceInputs = {
...
const coreProfiles = {
  P0: {
    id: 'P0-7R-balanced',
    computeTF: (32.768 + 262.144) * 8,
    powerW: 250 * 8,
    areaMm2: 5248,
    peakByCore: {L: 157.2864e12, H: 1258.2912e12, V: 78.6432e12, INDEXER: 78.6432e12, REDUCE: 78.6432e12}
  },
  P1: {
    id: 'P1-compact',
    computeTF: 176.9472 * 8,
    powerW: 237.46 * 8,
    areaMm2: 5248,
    peakByCore: {L: 157.2864e12, H: 1258.2912e12, V: 78.6432e12, INDEXER: 78.6432e12, REDUCE: 78.6432e12}
  }
...
const scale = read('data/workload/planning_operator_workload.json').operators;
...
function rowsFor(modelId, tp, physicalProfile, mcProfile, variation) {
  const rows = [];
  for (const [operatorId, coreClass, globalFlops, globalBytes, bytesClass] of scale[modelId]) {
    const flops = globalFlops * variation.compute / tp;
    const bytes = globalBytes * variation.bytes / tp;
    const bandwidth = bytesClass === 'collective' ? networkBandwidth * variation.network : mcProfiles[mcProfile] * variation.bandwidth;
    ...
    rows.push({
      ...
      requiredMemoryBandwidth: bytes * target,
      availableMemoryBandwidth: bandwidth,
      requiredToAvailableBandwidthRatio: (bytes * target) / bandwidth,
...
const directionalScore = read('data/direction/directional_tps_scorecard.json');
const candidates = [];
for (const physicalProfile of ['P0', 'P1']) {
  for (const mcProfile of ['MC320', 'MC640']) {
    for (const tp of [8, 16, 32]) {
      for (const model of manifest.models) {
        const operatorRows = rowsFor(model.modelId, tp, physicalProfile, mcProfile, {bandwidth: 1, computeCapacity: 1, compute: 1, bytes: 1, network: 1});
        ...
        candidates.push({
          ...
          candidateId: `${coreProfiles[physicalProfile].id}-${mcProfile}-TP${tp}`,
...
const selected = ['P0-7R-balanced-MC640-TP32', 'P0-7R-balanced-MC640-TP16', 'P1-compact-MC640-TP32'];
const selectedRows = candidates.filter(item => selected.includes(item.candidateId));
const sweepSummary = {
  complete: true,
  ...
  selectedSensitivity: sweep.filter(item => item.axes.bandwidth === 1 && item.axes.computeCapacity === 1 && item.axes.compute === 1 && item.axes.bytes === 1 && item.axes.network === 1)
};
const register = {
  schemaVersion: 'candidate-register-v0.2',
  updatedAt: '2026-09-22T00:00:00.000Z',
  sourceDirectionalRunId: 'stage-a-20260922-blocker-resolution',
  decisionState: 'D_GATE_PASSED',
  formalSelectedCandidates: selected,
  selectionBasis: {
    targetTpsPerUser: target,
    architectureGateTpsPerUser: profile.policy.sharedArchitectureGateTpsPerUser,
    policy: 'select at most three candidates; retain MC320 as reference and MC640 as manufacturing stretch',
    selected: selected.map(candidateId => candidates.find(item => item.candidateId === candidateId))
  },
  exploratorySweeps: [{
...
const direction = {
  schemaVersion: 'directional-tps-scorecard-v0.3',
  runId: 'stage-a-20260922-blocker-resolution',
  stage: 'direction',
  agentId: 'D7',
  targetTpsPerUser: target,
  architectureGateTpsPerUser: profile.policy.sharedArchitectureGateTpsPerUser,
  ...
  candidates,
  candidateSummaries: candidateIds.map(candidateId => {
  ...
  dGate: {
    areaConservation: packageSpec.compute.dieCount * packageSpec.compute.dieAreaMm2 + packageSpec.memory.cubeCount * packageSpec.memory.cubeAreaMm2Planning + packageSpec.packageTotals.placementRoutingReserveMm2 === packageSpec.placementWindow.areaMm2,
  ...
  blockers: [],
  nextActions: ['Run formal Stage B event model for the three selected candidates.', 'Close Q-Gate with shared manifest, event traces and provenance.']
};
...
write('data/governance/formal_manifest_binding.json', {
  ...
  boundCandidates: selected,
  status: 'BOUND_FOR_FORMAL_STAGE_B'
});
...
write('data/direction/directional_resource_envelope.json', {
...
const report = [
  ...
  '',
  `Run ID: \`${direction.runId}\``,
  `Manifest hash: \`${manifestHash}\``,
  '',
  '## Closed blockers',
  '',
  '- Three-model planning manifests are explicit and hash-bound.',
  `- Planning sweep: ${sweep.length} samples; K3/P0/MC320/TP32 only. This does not cover all D2-D6 physical axes.`,
  '- Formal candidate selection is recorded with three candidates.',
  '',
...
fs.writeFileSync(path.join(root, 'reports/direction/stage_a_blocker_resolution_20260922.md'), `${report}\n`, 'utf8');
console.log(JSON.stringify({runId: direction.runId, manifestHash, candidateCount: candidates.length, selected, sweepSamples: sweep.length, feasibleCount: sweepSummary.feasibleCount}, null, 2));
