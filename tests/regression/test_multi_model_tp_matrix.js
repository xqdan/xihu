...
const matrix = JSON.parse(fs.readFileSync(path.join(root, 'teams/model/inputs/multi_model_tp_matrix.json'), 'utf8'));
...
for (const testCase of matrix.cases) {
  ...
  // FFN/MoE is TP-only for every model: no expert parallelism, no all-to-all dispatch.
  assert.strictEqual(testCase.sharding.expertWeights, 'tensor_parallel');
  assert.strictEqual(testCase.expected.expertDispatch, false);
  assert(!testCase.features.includes('expert_parallelism'));
}
