...
const A=require('../../integration/detailed/k3_architecture_search.js'),S=require('../../integration/detailed/k3_b1_1000_search.js');
const close=(a,b)=>assert(Math.abs(a-b)<1e-7*Math.max(1,Math.abs(a),Math.abs(b)),`${a} != ${b}`);
const d=JSON.parse(fs.readFileSync('out/search/k3_b1_1000_results.json','utf8'));
assert.equal(d.batch,1);assert.equal(d.target,1000);assert.equal(d.tp,32);assert.equal(d.context,1048576);
const hash=crypto.createHash('sha256');for(const f of ['integration/detailed/k3_b1_1000_search.js','integration/detailed/k3_architecture_search.js','integration/detailed/k3_operator_sram_sim.js','teams/model/src/design_engine.js','out/search/k3_architecture_search_results.json'])hash.update(fs.readFileSync(f));assert.equal(d.inputHash,hash.digest('hex'));
let tests=0;
...
const html=fs.readFileSync('out/search/k3_b1_1000_report.html','utf8');
