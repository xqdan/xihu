...
const d=JSON.parse(fs.readFileSync('out/rdma/k3_b1_1000_rdma_sram_results.json','utf8'));
assert.equal(d.batch,1);assert.equal(d.target,1000);assert.equal(d.tp,32);
const h=crypto.createHash('sha256');for(const f of ['integration/detailed/k3_b1_1000_rdma_sram_search.js','integration/detailed/k3_sram_memory_rdma_model.js','integration/detailed/k3_architecture_search.js','integration/detailed/k3_operator_sram_sim.js','teams/model/src/design_engine.js'])h.update(fs.readFileSync(f));assert.equal(d.inputHash,h.digest('hex'));
...
const html=fs.readFileSync('out/rdma/k3_b1_1000_rdma_sram_report.html','utf8');assert(html.includes('直接读写远端 SRAM'));assert(html.includes('remote SRAM address'));assert(!/NaN|undefined|Infinity/.test(html));assert.equal((html.match(/<svg /g)||[]).length,1);
