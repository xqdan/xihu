...
const {evaluateQuantificationGate, observationCoverage, evaluateDirectionGate} = require('../../integration/governance/evaluate_gates');
...
// A candidate where one model misses the target cannot be formally selected (ADR-0008).
const belowTarget = score.candidateSummaries.find(s=>s.minTpsPerUser<score.targetTpsPerUser);
assert(belowTarget,'expected at least one candidate below target');
const withBelow = evaluateDirectionGate(env,score,{...register,formalSelectedCandidates:[...register.formalSelectedCandidates.slice(0,2),belowTarget.candidateId]});
assert.strictEqual(withBelow.selectionMeetsTarget,false);
...
const html = fs.readFileSync('out/dashboard/architecture_global_dashboard.html','utf8');
...
const feedback = read('out/governance/direction_feedback.json');
