...
const evidence=read('data/verification/timing_evidence_status.json');assert.strictEqual(evidence.current,'SYNTHETIC_BOTTLENECK_BOUND');assert.strictEqual(evidence.validatedSlots,0);assert.strictEqual(evidence.qGate,'BLOCKED');assert(evidence.exitCriteria.length>=4);
