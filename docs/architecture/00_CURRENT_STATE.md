...
> 已汇总为 [`21_TPS_DESIGN_BASELINE.md`](21_TPS_DESIGN_BASELINE.md)（ADR-0005，`teams/hardware/inputs/k3_mc_baseline.json#tpsDesign`）。

当前 Final Tuning 搜索候选的**权威数值**在
[`teams/hardware/inputs/k3_mc_baseline.json`](../../teams/hardware/inputs/k3_mc_baseline.json) 的 `computeDieCandidate` 中，
由 `npm run baseline:sync` 从
[`out/rdma/k3_rdma_final_tuning_results.json`](../../out/rdma/k3_rdma_final_tuning_results.json)
生成，`tests/regression/test_design_baseline.js` 强制两者一致。本文不再手抄数字，避免多处漂移。
...
[`integration/detailed/k3_physical_basis.js`](../../integration/detailed/k3_physical_basis.js)。重新搜索后移动到 8 L + 4 H
...
| 640 GB/s/MC | `modelResults.stretchMc640GBs` | Stretch 点（ADR-0019 归为 `STRETCH/AGGRESSIVE`）；是否达到 1000 以 `acceptance.currentStatus` 为准，且即使达到也只是模型结果 |
...
等待拖慢 shared 专家和 TMA 预取后可掩盖的时间变少（`tests/regression/test_design_baseline.js` 断言）。自 2026-09-25 起，640 GB/s 点的 raw 中
...
三星 SF4 面积、矩阵密度 3.2 TF/mm²、液冷（2026-09-25 决定，`integration/detailed/k3_physical_basis.js`）：
...
`teams/hardware/inputs/k3_mc_baseline.json#tauBasis.ceilingTpsByCount`。

### 3.1 多模型规划 TPS/usr（ADR-0006、ADR-0008）

规划链路（`npm run model:planning`）不再取“最慢单算子”的上界，改为规划 token 时间
（`integration/planning/token_time.js`）：
...
各系数都取自 K3 详细模型发布点（P1/MC640/TP32，1101.77）的时序分解（ADR-0008）：
...
| 模型 | dtype 口径 | TP32 / MC640（P1） | τ = 1.5 / 2.0 μs | 瓶颈 | 状态 |
| --- | --- | ---: | ---: | --- | --- |
...
- **正式候选：** 只有三个模型都 ≥ 1000 的槽位才能入选（ADR-0008），为 `P0-7R-balanced-MC640-TP32` 和 `P1-compact-MC640-TP32`。
  - 两者都是 τ 条件候选：K3 须 τ ≤ 约 1.35 μs（详细模型；规划模型为 1.44 / 1.41 μs）。
  - K3 的 1102.4 距 1050 架构门槛只有 5%。
- **Stage B 状态：** `PERFORMANCE_MISS_OUTSIDE_SELECTED_CANDIDATES_NOT_VALIDATED`，18 个可比槽位中有 11 个未达 1000。
- **非正式参考：** `P0-7R-balanced-MC320-TP32`（K3 551.2，未达目标）。
- **Stage B：** 按 `PLANNING_QUANTIFICATION` 运行。
- **Q-Gate：** 仍阻塞（无事件时序回放）。
...
| 冲突 | 当前处理 |
| --- | --- |
| P0 为 8 L + 8 H Core、1.0 GHz；P1 由搜索决定（当前 4 L + 5 H、1.0 GHz 固定） | 不是冲突，是两个 profile（ADR-0012）；P0 是物理主规划，P1 是当前可执行回归模型；报告必须注明 profile |
...
| 参考 MC 320 GB/s；默认搜索上限 480 GB/s；P1 最佳搜索使用 640 GB/s | `BLOCKER`，档位定义见 ADR-0019，必须选定可制造档 |
...
| 正式 manifest 曾与 `design_engine` preset 描述不同的 K3 | 已修正：K3 唯一来源是 `teams/model/src/design_engine.js#MODEL_PRESETS.kimiK3`，`tests/regression/test_k3_manifest_consistency.js` 强制一致 |
