...
版本：2026-09-22`n状态：`PROPOSED / INDUSTRIAL TEAM OPERATING MODEL`

## 1. 组织原则

项目按人类芯片公司的组织方式管理：三大专业团队并行产出，架构委员会负责跨团队决策，独立验证团队负责 Gate。D/Q 编号保留为流程角色，不再作为目录的唯一组织方式。

```text
Architecture Council / A0
        |
  +-----+------------------+
  |                        |
Hardware Team       Software Team       Model Team
规格与资源边界       部署与优化闭环        工作负载与测试场景
  |                        |                  |
...
| Team | 负责 | 不负责 |
|---|---|---|
| Hardware | 单芯片/封装规格、AI Core、SRAM/TMA、MC、NoC、Die-to-Die、PPA/RAS | 不把软件收益写成硬件能力；不定义模型真实性 |
...
| Council | 需求、架构方向、trade-off、ADR、D-Gate 决策 | 不替代专业团队实现细节 |
...
- 架构方向变化必须由 Council 记录 ADR；
- 模型配置未确认时只能是 `UNVERIFIED_PLANNING_MANIFEST`；
- 合成事件只能是 `SYNTHETIC_BOTTLENECK_BOUND`；
...
### Phase 1：Model + Hardware contract parallel
...
### Phase 2：Software feasibility parallel
...
`ARCH-02` 汇总三团队结果，生成候选架构、粗 Roofline、PPA 和 TPS envelope；`VV-01` 复核单位、候选覆盖和证据等级；Council 执行 D-Gate。
...
`VV-02` 验证事件守恒、依赖时序、e2e latency、18-slot 矩阵和 P0/P1/MC320/MC640 分离；若是 synthetic planning evidence，Q-Gate 必须保持 blocked。
...
团队 README 中定义各自文档 owner、输入、输出、约束、验收指标和交接格式；跨团队接口集中在 `docs/design/architecture/`，避免一份文档被多个团队同时改写。

## 7. 工业界式评审节奏

- 每周 Team Review：团队内部设计、风险、未决输入；
- 每周 Integration Review：模型—软件—硬件接口和指标变化；
- 每个 milestone 进行 Architecture Review Board：候选、PPA、软件收益、模型覆盖；
