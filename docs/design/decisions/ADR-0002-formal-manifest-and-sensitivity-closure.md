...
1. 建立 `data/workload/formal_model_manifests.json`，显式记录层数、dtype、MoE、KV/index state、MTP 和 operator inventory。
2. 该 manifest 是**架构规划冻结**，不是模型厂商配置或 silicon sign-off；GLM-5.2 和 DeepSeek-V4-Pro 的外部确认仍保留为 qualification risk。
3. 运行 `models/resolve_architecture_blockers.js` 生成 manifest hash、敏感性扫描、候选登记和 D-Gate 输入。
...
- `reports/direction/stage_a_blocker_resolution_20260922.md`
