...
2. **Derivation.** `deriveGlm()` in
   `scripts/generate_planning_operator_workload.js`; provenance status
...
3. **Check.**
   ...
   - `tests/test_k3_manifest_consistency.js` requires the ratio to be within
     0.5%.

4. **ASSUMPTION fields.** These are marked ASSUMPTION:
   - FP8 FlashMLA KV at 656 B (the HF reference caches BF16);
   - index keys at 132 B;
   - TP mapping without expert parallelism;
