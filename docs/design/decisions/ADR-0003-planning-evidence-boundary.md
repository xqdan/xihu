...
- Keep the calibrated K3 directional baseline unchanged. The planning operator
  workload for K3 is now DERIVED from src/core/design_engine.js (the single K3
  shape source) and must reconcile with that baseline within 25%
  (tests/test_k3_manifest_consistency.js). GLM-5.2 / DeepSeek-V4-Pro rows are
...
- Store the planning operator workload once (generated by
  scripts/generate_planning_operator_workload.js); both Stage A and B consume it.
...
- P0 and P1 carry distinct core-class peak capacities from
  models/planning/resource_profiles.js; they still share utilization and duty
...
- D-Gate decisions, the candidate register state and the candidate selection are
  computed (models/governance/evaluate_gates.js, selectCandidates policy in
  models/resolve_architecture_blockers.js); runners never write a decision literal.
