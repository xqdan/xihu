...
`docs/design/teams/software/SW-05_COLLECTIVE_STRATEGY_REVISION.md` reconciled the
...
This matters because `mappedPlan()` (src/search/k3_architecture_search.js)
rewrites the duration of every COMM op through `R.collective()`
(src/rdma/k3_sram_memory_rdma_model.js). The simulator's own `tauUs` (1.15 μs)
...
reference basis.** `OPT.countBasis` (src/rdma/k3_rdma_final_tuning_model.js) is
...
experts are latent-width (`expertInput: "latent"`, src/core/design_engine.js:43)
...
(docs/design/OPEN_ISSUES.md). If B-007 fails, the 92 delta loses its
...
first-class observable: `spec.tauBasis` in docs/design/spec/k3_mc_baseline.json,
...
- **Regression is possible in both directions.** `tests/test_k3_rdma_final_tuning.js`
...
node scripts/sync_baseline_spec.js
npm run report:rdma
npm test
```
