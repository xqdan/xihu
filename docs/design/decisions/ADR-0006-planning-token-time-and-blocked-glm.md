...
1. **Planning token time** (`models/planning/token_time.js`). It keeps the
...
2. **Calibration.** kMemory and kCompute are fitted once on the K3 detailed
   ...
   `data/rdma/k3_rdma_final_tuning_results.json#/search/best`). The fit is stored
   in `data/workload/planning_operator_workload.json#/calibration`.
...
- `tests/test_directional_units.js` and
  `tests/test_planning_evidence_dashboard.js` recompute every comparable slot
  from the workload and the stored calibration.
  `tests/test_k3_manifest_consistency.js` replays the calibration and the MC320
