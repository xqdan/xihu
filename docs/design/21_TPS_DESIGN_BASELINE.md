...
状态：`BASELINE`（由模型推导，不是 `FROZEN`，见 ADR-0003、[ADR-0005](decisions/ADR-0005-tps-design-baseline.md)）  
权威数值：[`spec/k3_mc_baseline.json#tpsDesign`](spec/k3_mc_baseline.json)，由 `npm run baseline:sync` 从
[`data/rdma/k3_rdma_final_tuning_results.json`](../../data/rdma/k3_rdma_final_tuning_results.json) 重算  
一致性检查：[`tests/test_tps_design_baseline.js`](../../tests/test_tps_design_baseline.js)
...
本文中的数字都是 `spec/k3_mc_baseline.json#tpsDesign` 的两位小数取值。测试会重算该块，并逐一核对本文数字；
模型一改，这里就必须同步，不能手改数字。02、03、05 号单元文档仍基于 2026-09-20 的候选；
凡与本文冲突的地方，一律以本文和 spec 为准。
...
- [`src/core/design_engine.js`](../../src/core/design_engine.js)
- [`src/simulation/k3_operator_sram_sim.js`](../../src/simulation/k3_operator_sram_sim.js)
- [`src/search/k3_architecture_search.js`](../../src/search/k3_architecture_search.js)
- [`src/rdma/k3_rdma_final_tuning_model.js`](../../src/rdma/k3_rdma_final_tuning_model.js)
- [`src/rdma/k3_tps_design_baseline.js`](../../src/rdma/k3_tps_design_baseline.js)
- [`src/search/k3_physical_basis.js`](../../src/search/k3_physical_basis.js)（物理基准：工艺面积、矩阵密度、散热限值）

## 1. 目标、预算与发布点

| 项目 | 值 | 状态 |
| --- | ---: | --- |
| 目标 | 1000 TPS/usr，B=1 decode，Context 1M，TP32，PP1 | `FROZEN`（ADR-001） |
...
| MC 带宽假设 | 640 GB/s/颗（ADR-011 的 `STRETCH/AGGRESSIVE` 档） | `BLOCKER`（B-002） |
...
频率也不参与成对微调。此前短暂发布的 1031.52 TPS/usr 点把频率降到 0.8 GHz，
把省下的功耗给了 H 矩阵；该点已撤回。那组硬件放回 1.0 GHz 会超出 Die 功耗和卡功耗上限，因此不可行。

恢复规则后的 N4 参考、风冷点是 1015.08 TPS/usr（4 L + 5 H core，UCIe 64 lane，卡功耗 2391.76 W / 2400 W）。

### 1.2 物理基准（2026-09-25 决定，ADR-0005 决定 6）

`A.physical()` 仍按 N4 参考系数和风冷限值计算，历史产物照常重放；
`src/search/k3_physical_basis.js` 的 `resize()` 在其结果上按 SF4 重算面积，并按液冷限值重做全部限值检查。
功耗、频率和带宽系数不变。

- 只换 SF4 面积：1015.08 的点变成 414.5 mm²，超出 400 mm²；重搜得 1015.06，Die 面积成为约束。
- 再把矩阵密度取 3.2 TF/mm²、限值换成液冷的 300 W / 2800 W：功耗让 UCIe 回到 128 lane（MC 不再被 UCIe 端口截断），
  面积和功耗一起买到更多 H 算力。两项单独放开，TPS 都只增加不到 10。
...
| 项目 | 规格 | 状态 |
| --- | --- | --- |
| 卡组织 | 8 Compute Die + 16 MC；每 Die 本地 2 MC；一张卡 = 一个 TP rank；32 张卡构成 TP32 | `BASELINE`（ADR-003） |
| 封装 | 7-reticle placement window 5248 mm²；当前占用 4589.71 mm² | `BASELINE`（ADR-010） |
...
不是可调旋钮（`tests/test_k3_rdma_final_tuning.js` 断言 OPT 中不存在这两个利用率键）。
...
- 在同一候选上，MC 低于 640 GB/s 就不达标（560 GB/s 为 977.00）；ADR-011 的默认上限 480 GB/s 只有 853.44。
...
1. **单一来源**：
   - 硬件取值在 `data/rdma/k3_rdma_final_tuning_results.json#search.best.x`；
   - 物理基准（工艺面积、矩阵密度、散热限值）在 `src/search/k3_physical_basis.js#BASIS`；
   - 软件开关在 `OPT`；
   - 经验因子在 `GAIN`；
   - 汇总在 `spec/k3_mc_baseline.json#tpsDesign`。
