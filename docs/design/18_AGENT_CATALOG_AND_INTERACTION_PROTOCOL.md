...
- `data/direction/workload_direction.json`；
...
- `data/direction/memory_bandwidth_envelope.json`；
...
- `data/direction/compute_core_envelope.json`；
...
- `data/direction/communication_envelope.json`；
...
- expert all-to-all、dispatch/combine 必须显式保留；
...
- `data/direction/package_7r_envelope.json`；
...
- `data/direction/software_gain_budget.json`；
...
- `data/direction/architecture_candidates.json`；
...
- `data/detailed/operator_ledgers/`；
- `data/detailed/roofline/`；
...
- `data/detailed/performance_results/`；
- `reports/detailed/fine_tps_matrix.*`；
- latency breakdown；
- P50/P95/P99；
...
- `verification/direction/` 和 `verification/detailed/`；
- gate report；
- regression report；
- report index；
- blocker list。
