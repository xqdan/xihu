...
| 优先级 | 杠杆 | 状态 | 归属 |
|---|---|---|---|
| **P0** | τ 基准显式化：为 `OPT.oneWayUs = 0.05 µs` 补物理推导，或改回 spec 的 1.15 µs 并重算全部 TPS | 2026-09-25 已改回 1.15 µs（`OPT.tauUs` 下限）并重算：681.06 TPS（加入 shared 专家重叠后 729.14，独立 TMA 通道后 774.77，KV 跨层预取与 DMA 抢占后 860.03，注意力/小算子映射与 H core 功耗重分配后 1007.27，FP8 KV cache 后 1031.52）；物理推导仍未做 | SW-05 + D4/B-004/B-005 |
...
ceiling(393)         = 760.58 TPS（仍低于 1000 目标；即使降到结构下限 209 也只有 937.03，τ 未收口前次数轴不可达）
                       2026-09-25 GAIN 置 1 后 compute 变大：720.19 / 876.47 TPS；加入 shared 专家重叠后 781.26 / 968.61 TPS；加入独立 TMA 通道后 845.72 / 1069.69 TPS；加入 KV 跨层预取与 DMA 抢占后 874.42 / 1116.02 TPS；加入注意力/小算子映射后 1033.19 / 1388.30 TPS；FP8 KV cache 后 1059.42 / 1436.08 TPS（spec.tauBasis；DMA 等待取 0、掩盖量按观测值固定，209 次的值是乐观上界）
