'use strict';
const fs=require('fs'),crypto=require('crypto'),{DEFAULT,MiB,build,simulate}=require('../src/simulation/k3_operator_sram_sim.js');
...
const payload={version:'2026-09-19',inputHash:crypto.createHash('sha256').update(fs.readFileSync('src/core/design_engine.js')).update(fs.readFileSync('src/simulation/k3_operator_sram_sim.js')).digest('hex'),defaultConfig:DEFAULT,sizes,plans,sweeps,details,sensitivities};
fs.writeFileSync('data/sram/k3_operator_sram_tps_results.json',JSON.stringify(payload,null,2)+'\n');
const template=fs.readFileSync('templates/sram/k3_operator_sram_report_template.html','utf8');
fs.writeFileSync('reports/sram/k3_operator_sram_tps_evaluation.html',template.replace('/*__REPORT_DATA__*/',JSON.stringify(payload).replace(/</g,'\\u003c')));
