...
const fs=require('fs'),vm=require('vm'),P=require('../src/core/sram_operator_peak.js'),E=require('../src/core/design_engine.js');
...
fs.writeFileSync('data/sram/sram_architecture_capacity.csv','\ufeff'+[header,...table].map(r=>r.map(c=>'"'+String(c).replaceAll('"','""')+'"').join(',')).join('\n')+'\n');
...
`;
fs.writeFileSync('reports/sram/sram_architecture_capacity.md',md);
