...
版本：2026-09-21（2026-09-26 按 ADR-0021 修订：只有一份硬件规格 P1，P0 相关问题关闭）  
...
**当前不具备架构签核条件。** 现有仓库已经有清晰的模块划分、18 个 TPS 观测槽位、唯一硬件规格 P1 和 K3 事件级模拟雏形；但三模型的规划 workload 尚未经事件级回放，TP8/TP16 仍被模拟器硬编码拒绝，算术强度/Roofline/算力 sizing 尚未形成闭环。因此现阶段的数字只能作为 architecture planning baseline，不能作为三模型达标或可制造性结论。
...
当前关键 TPS 证据是 E2 的 K3 TP32 两点（MC320 与 MC640，数值见 `00_CURRENT_STATE.md` 第 3.1 节和 `teams/hardware/inputs/k3_mc_baseline.json#modelResults`）；MC640 点在模型内超过 1050，但架构闸门还要求可制造 MC 路线与详细 tile 模型。其余 16 个观测槽位只有规划估算（E1），没有事件级结果。
...
| MR-002 | A0/A2/A3/A4 | ~~第二份手写规格（P0）未接入执行路径~~ 2026-09-26 关闭：只保留 Final Tuning 搜索得到的 P1（ADR-0021） | — | — | Q-Gate `singleHardwareSpec` 检查 |
...
| MR-006 | A0/A10 | ~~P1 结果不能外推 P0~~ 2026-09-26 关闭：只有一份规格（ADR-0021）；MC640 与 MC320 的区分见 MR-005 | — | — | — |
...
| MR-008 | A1/A7/A8 | ~~Expert dispatch/combine、all-to-all、capacity overflow、热点和 reroute 未事件级建模~~ 2026-09-25 不适用：FFN/MoE 为 TP-only，无 EP/all-to-all（ADR-0020） | DeepSeek/GLM 的主要瓶颈可能完全遗漏 | 增加 token routing trace、capacity factor、overflow、home mapping、combine protocol | P99 expert load/mean、overflow rate、remote bytes 和尾延迟均可复现 |
...
| MR-015 | A12/A13 | CI/报告未强制 model、TP、MC、physical profile、status、provenance 字段 | 可能出现数字存在但上下文缺失 | JSON Schema + negative tests + report gate | 缺字段、单位错误、第二份规格、pending 有数值均失败 |
| MR-016 | A3/A10 | 当前 BF16 peak（`computeDieCandidate.bf16DenseTflops`）是阵列预设的数学 peak，不是由三模型算术强度反推的 sizing | 芯片算力可能过配或欠配 | 用 operator roofline 反推 L/H/Vector/Indexer/Reduce 的 effective/peak 需求，并留 headroom | 形成 model×TP×operator 类别的 sizing 表，且可回溯到 FLOP/bytes |
...
- MODEL/STRETCH、PLANNING_ESTIMATE/MODEL_OBSERVED/SILICON_OBSERVED 的状态虽已写入文档，但缺少统一 CI policy。
...
- per-die、per-core、card aggregate 容量必须在 schema 中分层（`k3_mc_baseline.json#sramAccounting`）。
...
- endpoint 数量与 8 L + 4 H、SRAM slice、MC gateway、PHY 的物理对应关系未闭合。
...
- Die 300 W、卡 2800 W 是液冷假设上限（O-015），不是热/电/封装联合结果；VRM、RDL、MC 功耗和冷却路径缺少闭环。
...
| A2 | floorplan coordinate schema | 8 die + 16 MC 坐标、shoreline/keep-out 守恒 |
...
| A10 | 18-slot performance runner | 18×5 seeds，资源取自唯一规格，完整 provenance |
...
- 将 TP、MC raw/sustained 参数化。
- 完成 die/SRAM/NoC/floorplan 资源映射。
...
- 输出 TPS、breakdown、PPA、tail latency、failure mode。
...
primary_blockers = [MR-001, MR-003, MR-004, MR-010, MR-011, MR-016]
```
