...
旧 D1–D7、Q1–Q9 保留为 compatibility aliases，映射关系见 `teams/council/inputs/industrial_agent_organization.json`。
...
docs/architecture/contracts/  跨团队架构决策与接口
teams/vv/              独立验证、golden trace、Gate
out/agents/            Agent 组织可视化
```

团队 README 定义各自的文档 owner、输入、输出、约束、验收指标和交接格式；跨团队接口集中在 `docs/architecture/contracts/`，避免一份文档被多个团队同时改写。
