...
> **已应用到主线（2026-09-25，ADR-0004）**：三项差额已全部对齐到参考口径，`OPT.countBasis='reference-393'`，主线次数 510 → 393。其中 `Q / new-KV all-gather`（24）与 `Distributed sampling candidates`（1）**保留为本地算子**（同样字节与依赖边，不走网络），不是直接删除；`Shared output all-reduce`（92）并入 `Wup + Shared output all-reduce`。本节对账结论已被 `tests/regression/test_k3_rdma_final_tuning.js` 固化为双向守卫（393 / 510 均可复现）。
...
另有一个中间档可作参照：`out/rdma/k3_b1_1000_rdma_sram_results.json`（2026-09-19）实测 `commUs = 444.324 / 510 = 871 ns`。三个档位 1150 / 871 / 271 ns 之间没有任何书面论证，只有默认值的替换。
...
| 优先级 | 杠杆 | 状态 | 归属 |
|---|---|---|---|
| **P0** | τ 基准显式化：为 `OPT.oneWayUs = 0.05 µs` 补物理推导，或改回 spec 的 1.15 µs 并重算全部 TPS | 2026-09-25 已改回 1.15 µs（`OPT.tauUs` 下限）并重算：681.06 TPS（加入 shared 专家重叠后 729.14，独立 TMA 通道后 774.77，KV 跨层预取与 DMA 抢占后 860.03，注意力/小算子映射与 H core 功耗重分配后 1007.27，FP8 KV cache 后 1031.52，恢复固定 1.0 GHz 后 1015.08，三星 SF4 面积、矩阵密度 3.2 TF/mm² 与液冷上限后 1101.77）；物理推导仍未做 | SW-05 + D4/B-004/B-005 |
...
2. ~~**参考页的 393 口径是否已在本仓库登记**~~ **已结项**（2026-09-25）：`OPT.countBasis` 切换 + `spec.collectiveCount` 字段 + `tests/regression/test_k3_rdma_final_tuning.js` 双向守卫 + ADR-0004。
...
```
computeUs            = teams/hardware/inputs/k3_mc_baseline.json#modelResults.*.computeUs        = 779.7324474251067
commUs（发布候选）    = 同上 .commUs                                             = 138.42546670161298
平均单次（510 口径）  = commUs / 510                                             = 271.4 ns
平均单次（393 口径）  = teams/hardware/inputs/k3_mc_baseline.json#tauBasis.observedNsPerCollective
归约次数             = teams/hardware/inputs/k3_mc_baseline.json#collectiveCount.total            = 393
ceiling(N)           = 1e6 / ((computeUs + N×1.15) × 1.17)
ceiling(393)         = 760.58 TPS（仍低于 1000 目标；即使降到结构下限 209 也只有 937.03，τ 未收口前次数轴不可达）
                       2026-09-25 GAIN 置 1 后 compute 变大：720.19 / 876.47 TPS；加入 shared 专家重叠后 781.26 / 968.61 TPS；加入独立 TMA 通道后 845.72 / 1069.69 TPS；加入 KV 跨层预取与 DMA 抢占后 874.42 / 1116.02 TPS；加入注意力/小算子映射后 1033.19 / 1388.30 TPS；FP8 KV cache 后 1059.42 / 1436.08 TPS；恢复固定 1.0 GHz 后 1046.39 / 1412.24 TPS；SF4 面积、矩阵密度 3.2 TF/mm² 与液冷后 1134.46 / 1577.52 TPS（spec.tauBasis；DMA 等待取 0、掩盖量按观测值固定，209 次的值是乐观上界）
