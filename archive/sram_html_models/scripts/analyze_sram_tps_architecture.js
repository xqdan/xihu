...
function load(doubled=false){
 ...
 const c={window:{DesignEngine:require('../../../teams/model/src/design_engine.js')},document:{getElementById:id=>nodes[id]},console};vm.createContext(c);vm.runInContext(code,c);return c.window.audit;
}
...
fs.writeFileSync('archive/sram_html_models/data/sram_tps_architecture_analysis.json',JSON.stringify({source:'docs/sram/k3_sram_tps_model.html',context:1048576,tp:32,note:'Raw local-model sensitivity outputs; not validated operator-timed predictions.',records},null,2)+'\n');
