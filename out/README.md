...
|---|---|---|
| `workload/` | `planning_operator_workload.json` (planning operator rows, token-time calibration); `tps_observation_matrix.json` (three models × TP8/16/32 × MC320/640 × P0/P1 planning estimates; `null` means no event-level replay yet) | `generate_planning_operator_workload.js`, `stage_b.js` |
...
| `search/`, `rdma/`, `sram/` | K3 detailed search results and reports | `run_search.js`, `generate_k3_*.js` |
